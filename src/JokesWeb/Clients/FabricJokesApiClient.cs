...
namespace JokesWeb.Clients
{
    public class FabricJokesApiClient : IJokesApiClient
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            {
                languages.Add(
                    await this.GetResponseAsync<JokesLanguageModel>(
                        tuple.serviceName, 
                        tuple.partitionName, 
                        "api/jokes/language", 
                        cancellationToken));
        ...
        public async Task<IEnumerable<JokeModel>> GetJokesAsync(
            ...
            CancellationToken cancellationToken)
        {
            var (serviceName, partitionName) = await this.ResolveServiceAsync(language, category);

            return await this.GetResponseAsync<JokeModel[]>(serviceName, partitionName, "api/jokes", cancellationToken);
        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            foreach (var byLanguage in importJokes.GroupBy(importJoke => importJoke.Language))
            {
                ...
                {
                    var (serviceName, partitionName) = await this.ResolveServiceAsync(byLanguage.Key, byCategory.Key);
            ...
            string category)
        {
            ...
            foreach (var (serviceName, partitionName) in tuples)
            {
                var url = WebUtility.UrlDecode(serviceName.AbsoluteUri);
                if ((url.Contains($"/{language}/") || url.EndsWith($"/{language}")) && partitionName.Equals(category))
                {
                    return (serviceName, partitionName);
                }
            }
        ...
        private async Task<IEnumerable<(Uri serviceName, string partitionName)>> ResolveServicesAsync()
        {
            ...
            var services = await client.QueryManager.GetServiceListAsync(
                new Uri(this.context.CodePackageActivationContext.ApplicationName));
            ...
            CancellationToken cancellationToken)
        {
            ...
            var partition = await ServicePartitionResolver
               .GetDefault()
               .ResolveAsync(serviceName, new ServicePartitionKey(partitionName), cancellationToken);
